    ...
    <section id="reviews" className="py-24 bg-white overflow-hidden">
      <div className="container mx-auto px-4 md:px-6">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className="text-[#00AEEF] font-bold tracking-wider uppercase text-sm mb-3">{R.label}</h2>
          <h3 className="text-4xl md:text-5xl font-bold text-[#0A2A6E] font-serif mb-6">{R.heading}</h3>
          <p className="text-lg text-[#6B7280]">{R.body}</p>
        </div>

        <div className="relative">
          <div className="overflow-hidden cursor-grab active:cursor-grabbing" ref={emblaRef}>
            <div className="flex gap-6 py-4">
              {R.items.map((review, index) => (
                <div key={index} className="flex-[0_0_100%] sm:flex-[0_0_50%] lg:flex-[0_0_33.333%] min-w-0 pl-4">
                  <div className="bg-[#F4F7FB] rounded-2xl p-8 h-full border border-gray-100 hover:shadow-lg transition-shadow relative">
                    ...
                    <p className="text-[#0A2A6E] font-medium mb-8 leading-relaxed">"{review.text}"</p>
